document.addEventListener('DOMContentLoaded', () => {
            ...
            let map, temporaryMarker, institutionMarkers, institutionsCache = [];
            ...
            async function loadInstitutions() {
                try {
                    ...
                    if (response.status === 404) {
                        ...
                        institutionsCache = institutions.map(inst => {
                            try {
                                ...
                                console.error('Failed to parse localization for institution:', inst);
                                return null;
                            }
                        }).filter(Boolean); 
                    }
            ...
            function renderSidebarList(institutions) {
                ...
                if (institutions.length === 0) {
                    listEl.innerHTML = '<p class="text-center text-gray-500 mt-4">Nenhuma instituição cadastrada.</p>';
                    return;
                }
            ...
            function addInstitutionMarker(institution) {
                ...
                institution.marker = L.marker([lat, lng]).addTo(institutionMarkers).bindPopup(popupContent);
            }

            function openForm(mode = 'create', institution = null) {
                institutionForm.reset();
                if (mode === 'edit' && institution) {
                    editState = { isEditing: true, id: institution.id, originalCnpj: institution.cnpj };
            ...
            function updateFormLocation(lat, lng) {
                ...
                if (!temporaryMarker) {
                    ...
                    temporaryMarker.on('dragend', e => updateFormLocation(e.target.getLatLng().lat, e.target.getLatLng().lng));
                } else {
            ...
            async function handleFormSubmit(e) {
                ...
                const payload = {
                    name: data.name, cnpj: data.cnpj, contact: data.contact, description: data.description,
                    positionX: parseFloat(data.positionX), positionY: parseFloat(data.positionY)
                };
                ...
                try {
                    ...
                    showToast(`Instituição ${editState.isEditing ? 'atualizada' : 'criada'} com sucesso!`, 'success');
                    closeForm();
                    loadInstitutions();
                } catch (error) {
            ...
            toggleOpenBtn.addEventListener('click', () => sidebar.classList.remove('-ml-[100%]'));
            toggleCloseBtn.addEventListener('click', () => sidebar.classList.add('-ml-[100%]'));

            document.getElementById('search-btn').addEventListener('click', searchAddress);
            document.getElementById('address-search').addEventListener('keydown', e => { if (e.key === 'Enter') { e.preventDefault(); searchAddress(); } });
            ...
            async function searchAddress() {
                ...
                try {
                    const response = await fetch(`${NOMATIM_URL}?format=json&q=${encodeURIComponent(query)}&countrycodes=br`, { headers: { 'User-Agent': 'InstituicoesApp/1.0' } });
            ...
            function showToast(message, type = 'success') {
                ...
                toast.className = `fixed bottom-5 right-5 z-40 px-5 py-3 rounded-lg text-white ${type === 'success' ? 'bg-green-500' : 'bg-red-500'}`;
